...
test_sen1 = TEXT_A.preprocess(test_sen1)
test_sen1 = [[TEXT_A.vocab.stoi[x] for x in test_sen1]]

test_sen2 = TEXT_A.preprocess(test_sen2)
test_sen2 = [[TEXT_A.vocab.stoi[x] for x in test_sen2]]

with torch.no_grad():
    test_sen1 = np.asarray(test_sen1)
    test_sen2 = np.asarray(test_sen2)
    test_sen1 = torch.LongTensor(test_sen1)
    test_sen2 = torch.LongTensor(test_sen2)
    test_tensor1 = Variable(test_sen1)
    test_tensor2 = Variable(test_sen2)
    test_tensor1 = test_tensor1.cuda()
    test_tensor2 = test_tensor2.cuda()
    model.eval()
    output1, output2 = model(test_tensor1, test_tensor2)
    out1 = F.softmax(output1, 0)
    out2 = F.softmax(output2, 1)
    if (torch.argmax(out1[0]) == 0) and (torch.argmax(out2[0]) == 1):
        print("Correct")
    else:
        print("Wrong")
